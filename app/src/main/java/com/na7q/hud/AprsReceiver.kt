...
class AprsReceiver(private val updateHud: (String, String, String, String, String, String, Int, Int, String, String) -> Unit) : BroadcastReceiver() {
    override fun onReceive(context: Context?, intent: Intent?) {
        ...
        if (intent != null) {
			...
			val qrg = intent.getStringExtra("org.aprsdroid.app.QRG")?.takeIf { it.isNotEmpty() }?.let { "$it" + "MHz" } ?: ""

            // Log the received data
            Log.d("AprsReceiver", "Received data: source=$source, location=$location, callsign=$callsign, packet=$packet, qrg=$qrg")
            ...
            val model = if (parsedTocall != null) {
				...
				if (result == null) {
					result = AprsPacket.kenwoodtocall(comment.takeLast(1))
				}
            ...
            updateHud(source, location, callsign, packet, comment, symbol, speed, course, model ?: "", qrg)
        }
    }
}
